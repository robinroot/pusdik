...
class Dashboard extends CI_Controller {
	...
	public function update_profile(){
		...
		$id = 0;
		if ($status_berita == "sejarah") {
			$datas = array('id_profile' => 0,
							'sejarah' => $keterangan);
			$this->Mod_konfigurasi->update_profile($id,$datas);
			$sess = $this->session->set_flashdata('sukses', '<div class="alert alert-success alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Success Update Data!</strong></div>');
			redirect(base_url("dashboard/profil",$sess),'refresh');
		}elseif ($status_berita == "visimisi") {
			$datas = array('id_profile' => 0,
							'visimisi' => $keterangan);
			$this->Mod_konfigurasi->update_profile($id,$datas);
			$sess = $this->session->set_flashdata('sukses', '<div class="alert alert-success alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Success Update Data!</strong></div>');
			redirect(base_url("dashboard/profil",$sess),'refresh');
		}elseif ($status_berita == "tugaspokok") {
			$datas = array('id_profile' => 0,
							'tugaspokok' => $keterangan);
			$this->Mod_konfigurasi->update_profile($id,$datas);
			$sess = $this->session->set_flashdata('sukses', '<div class="alert alert-success alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Success Update Data!</strong></div>');
			redirect(base_url("dashboard/profil",$sess),'refresh');
		}else{
	...
	public function konfigurasi(){
		$this->load->model('Mod_konfigurasi');
		$data['configure'] = $this->Mod_konfigurasi->get_config();
		$data['title'] = 'Konfigurasi Umum';
		$this->load->view('backend/layout/vwSidebar', $data);
		$this->load->view('backend/layout/vwHeader');
		$this->load->view('backend/dashboard/konfigurasi',$data);
		$this->load->view('backend/layout/vwFooter');
	}

	public function konfigurasi_quotes(){
		$this->load->model('Mod_konfigurasi');
		$data['configure'] = $this->Mod_konfigurasi->get_config();
		$data['title'] = 'Konfigurasi Quotes';
		$data['ambilquote'] = $this->Mod_konfigurasi->get_quotes();
		$this->load->view('backend/layout/vwSidebar', $data);
		$this->load->view('backend/layout/vwHeader');
		$this->load->view('backend/dashboard/konfigurasi_quotes',$data);
		$this->load->view('backend/layout/vwFooter');
	}
	...
	public function update_configure(){
		...
		$data = array(
				  ...
				  'twitter' => $i->post('twitter', TRUE),
				  'instagram' => $i->post('instagram', TRUE)
				);

	$this->Mod_konfigurasi->update_config($data, $id);
			redirect(base_url('dashboard/konfigurasi'));
			exit;
	}
	...
	public function post_quote(){
		$i = $this->input;
		$id = 1;
		$data = array(	'id_quote' => 1,
						'quote' => $i->post("quote", TRUE),
						'quote1' => $i->post("quote1", TRUE),
						'quote2' => $i->post("quote2", TRUE),
						'quote3' => $i->post("quote3", TRUE)
			);
		$this->Mod_konfigurasi->post_quote($id,$data);
		$sess = $this->session->set_flashdata('sukses', '<div class="alert alert-success alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Success Update Data!</strong></div>');
			redirect(base_url("dashboard/konfigurasi_quotes",$sess),'refresh');
	}
	...
	public function show_account(){
		$data =	array(
			'list' => $this->Mod_konfigurasi->list_account(),
			'title' => "List Akun"
		);
		$this->load->view('backend/layout/vwSidebar', $data);
		$this->load->view('backend/layout/vwHeader');
		$this->load->view('backend/dashboard/vwAccount',$data);
		$this->load->view('backend/layout/vwFooter');
	}
	...
	public function add_account(){
		...
		$row=$query->num_rows();
		 ...
		 if($row)
		 {
		 $this->session->set_flashdata('gagal','<div class="alert alert-danger"><strong>GAGAL</strong>Username sudah terdaftar</div>');
		 redirect('dashboard/show_account','refresh');
		 exit;
		 }else{
			 $data = array('id_user' => '',
	 									...
	 									'akses_level' => $this->input->post('level')
	 								);
		 $this->db->insert('users',$data);

		$this->session->set_flashdata('berhasil','<div class="alert alert-success"><strong>Behasil tambah user</strong></div>');
		redirect('dashboard/show_account');
		exit();
		 }
	...
	public function edit_account($id_user){
		$i = $this->input;
		$data = array('nama' => $i->post('nama_user'),
					  'email' => $i->post('email'),
						'akses_level' => $i->post('level')
					);
		$this->Mod_konfigurasi->update_akun($data, $id_user);
				redirect(base_url('dashboard/show_account'));
				exit;
	}

	public function delete_account($id_user){
		$data = array('id_user'	=> $id_user);
		$this->Mod_konfigurasi->delete_account($data);
		$this->session->set_flashdata('sukses','Kategori berita telah didelete');
		redirect('dashboard/show_account');
		exit;
	}
	...
	function check_account(){
		...
		$cek=$this->Mod_konfigurasi->Getuser(array('password' => $old_password,'username'=>$username));
		if($cek->num_rows() > 0){
	...
	public function savechange(){
		...
		redirect('dashboard/viewProfile','refresh');
		exit;
	}
	...
	public function viewProfile(){
		...
		$this->load->view('backend/layout/vwSidebar', $data);
		$this->load->view('backend/layout/vwHeader');
		$this->load->view('backend/dashboard/vwProfile',$data);
		$this->load->view('backend/layout/vwFooter');
	}
